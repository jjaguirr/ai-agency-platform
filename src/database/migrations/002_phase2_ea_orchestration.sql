-- AI Agency Platform - Phase 2 Database Schema Updates
-- EA Orchestration System with Premium-Casual Personality and Cross-Channel Context
-- Version: 2.0 - Phase 2 EA Evolution
-- Migration: 002_phase2_ea_orchestration
...
CREATE TABLE IF NOT EXISTS conversation_context (
    ...
    -- Context content with vector search capability
    ...
    -- Vector embedding for semantic search (OpenAI ada-002 compatible)
...
CREATE TABLE IF NOT EXISTS conversation_context_transitions (
    ...
    -- Foreign key to main context table
    FOREIGN KEY (context_id, customer_id) REFERENCES conversation_context(context_id, customer_id) ON DELETE CASCADE
);
...
-- Vector search index for conversation context
CREATE INDEX IF NOT EXISTS idx_conversation_context_embedding ON conversation_context USING ivfflat (context_embedding vector_cosine_ops) WITH (lists = 100);
...
INSERT INTO schema_migrations (version, description) VALUES 
    ('2.0.0', 'Phase 2 EA Orchestration System - Premium-Casual Personality, Cross-Channel Context, Personal Brand Metrics, Voice Integration')
