...
class ExecutiveAssistantMemory:
    """
    Per-customer working memory.

    Redis (db=hash%16) holds active conversation context. Postgres holds
    business context. The semantic vector layer (Mem0/Qdrant) was never
    wired into the live system and has been removed; store/search methods
    are kept as no-op stubs because callers swallow None/[] gracefully.
    """
    ...
    async def store_business_knowledge(self, knowledge: str, metadata: Dict):
        """No-op stub. Semantic store removed; callers tolerate None."""
        logger.debug(f"store_business_knowledge (no-op): {knowledge[:80]}")
        return None

    async def search_business_knowledge(self, query: str, limit: int = 10) -> List[Dict]:
        """No-op stub. Semantic search removed; callers tolerate empty list."""
        return []
