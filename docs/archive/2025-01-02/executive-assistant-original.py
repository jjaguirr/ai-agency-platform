...
class ExecutiveAssistantMemory:
    ...
    def __init__(self, customer_id: str):
        ...
        # Configure Mem0 with proper backends and customer-specific user_id
        mem0_config = {
            "vector_store": {
                "provider": "chroma",
                "config": {
    ...
    async def store_business_knowledge(self, knowledge: str, metadata: Dict):
        ...
        try:
            # Use Mem0's add method with customer-specific user_id for isolation
    ...
    async def search_business_knowledge(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            # Use Mem0's search with customer-specific user_id for isolation
    ...
    def _create_conversation_graph(self) -> StateGraph:
        ...
        async def create_workflow_node(state: ConversationState) -> ConversationState:
            ...
            if state.needs_workflow and not state.workflow_created:
                ...
                try:
                    workflow_result = await create_workflow(process_description, "")
                    state.messages.append(AIMessage(content=workflow_result))
                    state.workflow_created = True
                except Exception as e:
                    logger.error(f"Error creating workflow: {e}")
                    state.messages.append(AIMessage(content="I'm working on creating that workflow for you. Let me get the details sorted out."))
            ...
            if state.messages:
                ...
                if isinstance(last_message, HumanMessage):
                    ...
                    if "business" in content or "company" in content:
                        ...
                        try:
                            await store_business_insight(last_message.content, "business_info")
                        except Exception as e:
