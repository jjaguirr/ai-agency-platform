...
async def aclient(app):
    """ASGI transport — shares the test's event loop so fakeredis
    populated in an async test body is visible to the request."""
...
class TestWorkflowsEndpoint:
    ...
    async def test_response_schema(self, aclient, store, token_a):
    ...
    async def test_tenant_scoped(self, aclient, store, token_a, token_b):
        await store.add_workflow("cust_a", "wf_a", "A's Report", "active")
        await store.add_workflow("cust_b", "wf_b", "B's Sync", "active")

        resp_a = await aclient.get("/v1/workflows",
                                   headers={"Authorization": f"Bearer {token_a}"})
        resp_b = await aclient.get("/v1/workflows",
