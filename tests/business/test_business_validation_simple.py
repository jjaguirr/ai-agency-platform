...
class TestEABusinessValidation:
    ...
    @pytest.mark.asyncio 
    async def test_business_discovery_conversation(self, business_intelligence_evaluator, evaluation_mode):
        """
        Business Requirement: EA learns business through conversation
        ENHANCED: Now uses semantic evaluation instead of keyword matching
        """
        ea = ExecutiveAssistant(customer_id="business_discovery")
        
        # Customer describes their business
        business_description = """
        I run a jewelry e-commerce store called Sparkle & Shine. 
        We sell handcrafted jewelry through Instagram and our website.
        I spend 3 hours daily on social media posts and 5 hours weekly creating invoices.
        ...
        # LEGACY: Keyword-based validation for comparison
        response_lower = response.lower()
        business_elements = [
            "jewelry", "e-commerce", "instagram", "social media", 
            "automat", "invoice", "sparkle", "tasks"
        ]
        keyword_score = sum(1 for element in business_elements if element in response_lower)
        
        # NEW: Semantic business understanding evaluation
        if evaluation_mode["real_evaluation_available"]:
            # Real AI semantic evaluation
            semantic_assessment = await business_intelligence_evaluator.validate_business_understanding_async(
                business_description=business_description,
                ea_response=response,
                conversation_history=[]
            )
            
            # Use semantic evaluation as primary validation
            assert semantic_assessment.passed, f"Semantic evaluation failed: {semantic_assessment.reasoning}"
            assert semantic_assessment.score >= 0.6, f"Semantic understanding score {semantic_assessment.score} < 0.6"
            
            # Report comparison
            print(f"✅ SEMANTIC Understanding: {semantic_assessment.score:.2f} (confidence: {semantic_assessment.confidence})")
            print(f"📊 Legacy keyword score: {keyword_score}/8 ({keyword_score/8:.1%})")
            print(f"🔍 Evaluation mode: Real AI semantic validation")
            
            # Return semantic assessment for downstream use
            return {
                "semantic_score": semantic_assessment.score,
                "keyword_score": keyword_score,
                "evaluation_type": "semantic",
                "assessment": semantic_assessment
            }
        else:
            # Fallback to keyword matching with warning
            assert keyword_score >= 4, f"EA business understanding: {keyword_score}/8 (< 50%)"
            
            print(f"⚠️  FALLBACK: Keyword matching score: {keyword_score}/8")
    ...
    @pytest.mark.asyncio
    async def test_automation_opportunity_identification(self, business_intelligence_evaluator, evaluation_mode):
        """
        Business Requirement: EA identifies automation opportunities
        ENHANCED: Now uses semantic evaluation instead of keyword matching
        """
        ea = ExecutiveAssistant(customer_id="automation_test")
        
        automation_scenarios = [
            {
                "business": "I manually post to social media 3 times daily and create invoices weekly",
                "pain_points": ["manual social media posting", "invoice creation"],
                "keywords": ["social", "media", "post", "invoice", "automat", "workflow"]
            },
            {
                "business": "I spend hours responding to customer emails and scheduling appointments",
                "pain_points": ["email responses", "appointment scheduling"],
                "keywords": ["email", "customer", "schedule", "appointment", "automat", "response"]
            }
        ]
        ...
        for scenario in automation_scenarios:
            ...
            legacy_success = keyword_matches >= 2
            
            # NEW: Semantic automation opportunity evaluation
            if evaluation_mode["real_evaluation_available"]:
                # Real AI semantic evaluation
                automation_assessment = await business_intelligence_evaluator.validate_automation_opportunities_async(
                    business_context=scenario["business"],
                    ea_recommendations=response,
                    pain_points=scenario["pain_points"]
                )
                
                semantic_success = automation_assessment.passed and automation_assessment.score >= 0.6
                if semantic_success:
                    successful_identifications += 1
                    
                semantic_results.append({
                    "scenario": scenario["business"][:50] + "...",
                    "semantic_score": automation_assessment.score,
                    "semantic_passed": semantic_success,
                    "keyword_matches": keyword_matches,
                    "legacy_passed": legacy_success
                })
            else:
        ...
        identification_rate = successful_identifications / len(automation_scenarios)
        
        # Report results based on evaluation mode
        if evaluation_mode["real_evaluation_available"]:
            assert identification_rate >= 0.5, f"Semantic automation identification: {identification_rate:.1%} < 50%"
            
            print(f"✅ SEMANTIC Automation identification: {identification_rate:.1%}")
            print(f"🔍 Evaluation mode: Real AI semantic validation")
            
            for result in semantic_results:
                print(f"📊 {result['scenario']}: Semantic={result['semantic_score']:.2f}, Keywords={result['keyword_matches']}")
                
        else:
            assert identification_rate >= 0.5, f"Automation identification: {identification_rate:.1%} < 50%"
            
            print(f"⚠️  FALLBACK: Keyword-based identification: {identification_rate:.1%}")
            print(f"🔍 Evaluation mode: Mock/keyword fallback (no OpenAI API key)")
        
        return {
            "identification_rate": identification_rate,
            "evaluation_type": "semantic" if evaluation_mode["real_evaluation_available"] else "keyword_fallback",
