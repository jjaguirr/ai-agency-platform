...
class TestEABusinessValidation:
    ...
    async def test_business_discovery_conversation(self):
        ...
        Semantic-eval branch removed — its fixtures (business_intelligence_evaluator,
        evaluation_mode) were stripped from conftest in a716c65. Keyword-match
        assertion below was the no-API-key fallback path; it's now the sole path.
        """
        ...
        print(f"✅ Business discovery keyword score: {keyword_score}/8")
    ...
    @pytest.mark.asyncio
    async def test_automation_opportunity_identification(self):
        """
        Business Requirement: EA identifies automation opportunities.

        Semantic-eval branch removed — fixtures stripped in a716c65. Keyword-match
        path below was the no-API-key fallback; it's now the sole path.
        """
        ...
        print(f"✅ Automation identification rate: {identification_rate:.1%}")
